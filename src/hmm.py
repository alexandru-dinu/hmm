...
def forward(observations: np.ndarray, hmm: HMM) -> Tuple[float, np.ndarray]:
    T = len(observations)
...
def backward(observations: np.ndarray, hmm: HMM) -> Tuple[float, np.ndarray]:
    T = len(observations)
...
def viterbi(observations: np.ndarray, hmm: HMM) -> Tuple[np.ndarray, np.ndarray]:
    T = len(observations)
...
def expectation(samples: List[np.ndarray], hmm: HMM) -> Tuple[List[np.ndarray], List[np.ndarray], float]:
    ...
    for l in range(L):
        ...
        p, alpha = forward(samples[l], hmm)
        _, beta = backward(samples[l], hmm)
...
def baum_welch(N, M, samples, num_it=None, plot=True) -> HMM:
    ...
    gammas, xis, logp = expectation(samples, hmm)
