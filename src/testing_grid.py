...
def test_forward_algorithm(grids):
    ...
    for grid in grids:
        p, _ = forward(grid.states_no, observations, grid.get_hmm())
...
def test_alpha_values(grid, observations, test_values):
    ...
    p, alpha = forward(grid.states_no, observations, grid.get_hmm())
...
def test_viterbi(grid, observations, test_states, test_values):
    ...
    H, W = grid.shape
    states, delta = viterbi(grid.states_no, observations, grid.get_hmm())
    states = [(s // W, s % W) for s in states]
